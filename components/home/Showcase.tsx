...
export default function Schowcase(){
    ...
    return(
        <section className="mt-[50px]">
            <h1 className="text-center text-[3.4em] font-semibold mb-9">Nedávné práce</h1>
            
            <div className="flex flex-wrap justify-center gap-6">
                {!isLoading && projects.slice(0,3).map(proj => (
                    <div key={proj._id} className="bg-primary rounded-[5px] shadow-[1px_1px_10px_#00000033] p-[15px_15px_10px_15px] w-[400px]">
                        <div className="overflow-hidden w-full">
                            <Image src={proj.imageUrl} alt={proj.name} width={1500} height={1500} className="rounded-[5px] w-full h-auto max-h-[13rem] mb-3 select-none" draggable={false}/>
                        </div>
